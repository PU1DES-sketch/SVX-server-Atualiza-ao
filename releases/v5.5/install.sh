...
sql_old = 'txt = set_value(txt, "Rx1", "SQL_HANGTIME", hang_time)'
sql_new = 'txt = set_value(txt, "Rx1", "SQL_HANGTIME", "0")'
if sql_new not in apply_text:
    if sql_old in apply_text:
        apply_text = apply_text.replace(sql_old, sql_new)
    elif rgr_line in apply_text:
