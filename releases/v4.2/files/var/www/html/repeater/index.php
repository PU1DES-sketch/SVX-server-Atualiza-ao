...
<body>
    <div id="sidebar">
        ...
        <div class="menu-item <?php echo $activePage === 'config' && !$canEdit ? 'active' : ''; ?>" onclick="showPage('config', this)">CONFIGURACOES</div>
        <div class="menu-item <?php echo $canEdit ? 'active' : ''; ?>" onclick="<?php echo $isLogged ? "window.location.href='index.php?page=apply'" : "window.location.href='login.php?next=index.php%3Fpage%3Dapply'"; ?>">APLICAR CONFIGURACOES</div>
        <div class="menu-item" onclick="location.reload()">ATUALIZAR PAINEL</div>
        <div class="menu-item" onclick="window.location.href='<?php echo $isLogged ? 'login.php?logout=1' : 'login.php?next=index.php%3Fpage%3Dapply'; ?>'"><?php echo $isLogged ? 'SAIR DO PAINEL' : 'ENTRAR NO PAINEL'; ?></div>
    ...
    <div id="main">
        ...
        <div id="config" class="page <?php echo $activePage === 'config' ? 'active' : ''; ?>">
            ...
            <?php if ($canEdit): ?>
                <div class="note">Modo de edicao liberado. Ao salvar, o painel vai pedir a senha novamente para confirmar a aplicacao das configuracoes.</div>
            <?php else: ?>
                <div class="note">Modo somente leitura. Aqui qualquer pessoa pode ver como o repetidor esta configurado, mas nada pode ser salvo ou alterado.</div>
            <?php endif; ?>

            <div class="box">
                <h3><?php echo $canEdit ? 'Aplicacao de Configuracoes' : 'Configuracoes da Repetidora'; ?></h3>
